<div>
    ...
    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl p-6 mb-8 text-white">
        <div class="text-sm opacity-80 mb-1">Total utestående</div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800">
                            <tr>
                                <th wire:click="sort('invoice_number')" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider cursor-pointer hover:text-zinc-700 dark:hover:text-zinc-200">
                                    Fakturanr.
                            ...
                            @foreach($invoices as $invoice)
                                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('invoices.show', $invoice) }}" class="font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                            {{ $invoice->invoice_number }}
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-zinc-900 dark:text-white">{{ $invoice->customer_name }}</div>
                                        @if($invoice->contact)
                                            <div class="text-sm text-zinc-500">{{ $invoice->contact->company_name }}</div>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($invoice->invoiceStatus)
                                            <flux:badge color="{{ $invoice->invoiceStatus->color ?? 'zinc' }}">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <flux:heading size="lg" level="3" class="text-zinc-900 dark:text-white mb-2">
                        Ingen utestående fordringer
                    </flux:heading>
                    <flux:text class="text-zinc-600 dark:text-zinc-400">
                        Alle kundefakturaer er betalt
                    </flux:text>
                </div>
            @endif
        </div>
