        ...
        <div class="flex-1 flex justify-center items-center p-8">
            <div class="w-full max-w-sm space-y-8">
                ...
                <div class="text-center">
                    <flux:heading size="xl">Tofaktorautentisering</flux:heading>
                    <flux:subheading class="mt-2">Bekreft innloggingen din</flux:subheading>
                </div>

                <div x-data="{ recovery: false }">
                    {{-- Authenticator Code Form --}}
                    <form method="POST" action="{{ route('two-factor.login') }}" class="space-y-6" x-show="!recovery">
                        @csrf

                        <div class="p-4 bg-zinc-50 dark:bg-zinc-800 rounded-xl">
                            <div class="flex items-start gap-3">
                                <flux:icon.device-phone-mobile class="h-5 w-5 text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0" />
                                <flux:text class="text-sm text-zinc-600 dark:text-zinc-400">
                                    Skriv inn engangskoden fra autentiseringsappen din for å bekrefte at det er deg.
                                </flux:text>
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            <flux:link href="#" @click.prevent="recovery = true" variant="subtle" class="text-sm">
                                Bruk en gjenopprettingskode
                            </flux:link>
                        </div>
                    </form>

                    {{-- Recovery Code Form --}}
                    <form method="POST" action="{{ route('two-factor.login') }}" class="space-y-6" x-show="recovery" x-cloak>
                        @csrf

                        <div class="p-4 bg-zinc-50 dark:bg-zinc-800 rounded-xl">
                            <div class="flex items-start gap-3">
                                <flux:icon.key class="h-5 w-5 text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0" />
                                <flux:text class="text-sm text-zinc-600 dark:text-zinc-400">
                                    Skriv inn en av gjenopprettingskodene dine for å logge inn.
                                </flux:text>
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            <flux:link href="#" @click.prevent="recovery = false" variant="subtle" class="text-sm">
                                Bruk autentiseringskode
                            </flux:link>
                        </div>
                    </form>
                </div>
                ...
                <flux:subheading class="text-center">
                    Problemer? <flux:link href="mailto:support@konradoffice.no">Kontakt support</flux:link>
                </flux:subheading>
            </div>
        </div>
    </div>
