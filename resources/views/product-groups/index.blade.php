<x-layouts.app title="Varegrupper">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <flux:text class="mt-1 text-base text-zinc-600 dark:text-zinc-400">
                            Administrer varegrupper for produkter
                        </flux:text>
                    </div>
                </div>
                <flux:button href="{{ route('product-groups.create') }}" variant="primary" class="px-6 py-3 shadow-lg shadow-violet-500/30">
                                ...
                                <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                                    @foreach($productGroups as $group)
                                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <flux:badge variant="outline">{{ $group->code }}</flux:badge>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <flux:badge variant="{{ $group->is_active ? 'success' : 'outline' }}">
                                                    {{ $group->is_active ? 'Aktiv' : 'Inaktiv' }}
                                                </flux:badge>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex items-center justify-end gap-2">
                                                    <flux:button href="{{ route('product-groups.edit', $group) }}" variant="ghost" size="sm">
                                                        <flux:icon.pencil class="w-4 h-4" />
                                                    </flux:button>
                                                    <form action="{{ route('product-groups.destroy', $group) }}" method="POST" class="inline" onsubmit="return confirm('Er du sikker på at du vil slette denne varegruppen?')">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <flux:heading size="lg" level="3" class="text-zinc-900 dark:text-white mb-2">
                                Ingen varegrupper funnet
                            </flux:heading>
                            <flux:text class="text-zinc-600 dark:text-zinc-400 mb-6">
                                Kom i gang ved å opprette din første varegruppe
                            </flux:text>
                            <flux:button href="{{ route('product-groups.create') }}" variant="primary">
                                <flux:icon.plus class="w-5 h-5 mr-2" />
                                Opprett varegruppe
                            </flux:button>
                        </div>
