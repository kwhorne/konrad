<x-layouts.app title="Ny momssats">
                ...
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        ...
                        <flux:text class="mt-1 text-zinc-600 dark:text-zinc-400">
                            Opprett en ny momssats
                        </flux:text>
                    </div>
                </div>

                <flux:card class="bg-white dark:bg-zinc-900 shadow-lg border border-zinc-200 dark:border-zinc-700">
                    <form action="{{ route('vat-rates.store') }}" method="POST" class="p-6 space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <flux:field>
                                ...
                                <flux:input id="code" name="code" type="text" value="{{ old('code') }}" required placeholder="F.eks. MVA25" />
                                @error('code')
                                    <flux:error>{{ $message }}</flux:error>
                                @enderror
                            </flux:field>
                        ...
                        <flux:field>
                            ...
                            <flux:input id="rate" name="rate" type="number" step="0.01" min="0" max="100" value="{{ old('rate') }}" required />
                            @error('rate')
                                <flux:error>{{ $message }}</flux:error>
                            @enderror
                        </flux:field>
                        ...
                        <div class="flex gap-6">
                            ...
                            <flux:field>
                                <flux:checkbox id="is_default" name="is_default" value="1" label="Standard momssats" checked="{{ old('is_default', false) }}" />
                            </flux:field>
                        </div>
