                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <flux:text class="mt-1 text-base text-zinc-600 dark:text-zinc-400">
                            Administrer enheter for produkter
                        </flux:text>
                    </div>
                </div>
                <flux:button href="{{ route('units.create') }}" variant="primary" class="px-6 py-3 shadow-lg shadow-blue-500/30">
                                ...
                                <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                                    @foreach($units as $unit)
                                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <flux:badge variant="outline">{{ $unit->code }}</flux:badge>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <flux:badge variant="{{ $unit->is_active ? 'success' : 'outline' }}">
                                                    {{ $unit->is_active ? 'Aktiv' : 'Inaktiv' }}
                                                </flux:badge>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex items-center justify-end gap-2">
                                                    <flux:button href="{{ route('units.edit', $unit) }}" variant="ghost" size="sm">
                                                        <flux:icon.pencil class="w-4 h-4" />
                                                    </flux:button>
                                                    <form action="{{ route('units.destroy', $unit) }}" method="POST" class="inline" onsubmit="return confirm('Er du sikker på at du vil slette denne enheten?')">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <flux:text class="text-zinc-600 dark:text-zinc-400 mb-6">
                                Kom i gang ved å opprette din første enhet
                            </flux:text>
                            <flux:button href="{{ route('units.create') }}" variant="primary">
                                <flux:icon.plus class="w-5 h-5 mr-2" />
                                Opprett enhet
                            </flux:button>
                        </div>
